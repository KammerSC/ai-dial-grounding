...
SYSTEM_PROMPT = """
You are a RAG-powered assistant that answers questions about user information.
...
class UserRAG:
    ...
    async def _create_vectorstore_with_batching(self, documents: list[Document], batch_size: int = 100):
        ...
        batched_tasks = [FAISS.afrom_documents(batch, self.embeddings) for batch in batches]
        # 3. Gather tasks with asyncio
        batched_results = await asyncio.gather(*batched_tasks, return_exceptions=True)
        ...
        for batch_vectorstore in batched_results:
        ...
        # 6. Return `final_vectorstore` (whats the 5th step?)
